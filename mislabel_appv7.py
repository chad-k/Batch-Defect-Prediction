...
**This app includes automatic Part Number filtering in the sidebar.**

Different part numbers have **different specifications and tolerances**. Mixing them creates false alarms:

**❌ Problem (mixing part numbers):**
```
Batch B001 contains:
  Part A (spec: 10.0 ± 0.5):  readings 9.8, 10.1, 10.0
  Part B (spec: 20.0 ± 1.0):  readings 19.5, 20.5, 20.0
  Part C (spec: 5.0 ± 0.2):   readings 4.9, 5.1, 5.0

All mixed → Range: 4.9 to 20.5 (HUGE!)
→ Everything flagged as anomalous ❌
```

**✅ Solution (filter by part number):**
```
Analyze only Part A:
  readings: 9.8, 10.1, 10.0
  Range: 9.8 to 10.1 (normal)
  Accurate detection ✓
```
...
5. To analyze another part, change the selector
...
if mode == "Upload CSV":
    ...
    demo = []
    # Define part numbers for demo
    part_numbers = ["PN-A", "PN-B", "PN-C"]
    
    for b in range(1, 301):
        batch = f"B{b:05d}"
        # Assign part number (cycles through the 3 types)
        part_no = part_numbers[b % len(part_numbers)]
        
        # Different specs for different parts
        if part_no == "PN-A":
        ...
        for r in range(row_ct):
            rec = {
                ...
                "PartNo": part_no,  # ✅ ADDED: Part number column
                "Batch": batch,
...
# ----------------------------
# Part Number Filter (CRITICAL for multi-part datasets)
# ----------------------------
st.sidebar.header("Product Filter")

# Detect part number column (common names)
...
if part_number_cols:
    part_col = st.sidebar.selectbox(
        ...
        help="Column containing part numbers/SKUs. Each part type has different specs, so analyzing per-part ensures accurate anomaly detection."
    )
else:
    # Auto-detection failed, show all columns for manual selection
    st.sidebar.warning("⚠️ Could not auto-detect part number column. Please select it manually from the dropdown below.")
    st.sidebar.write("**Available columns:**")
    st.sidebar.write(", ".join([f"`{c}`" for c in all_cols]))
    
    part_col = st.sidebar.selectbox(
        "Select Part Number/SKU Column (Manual)",
        options=all_cols,
        help="Which column contains your part numbers/SKUs? (e.g., PartNo, ProductType, ItemNumber, etc.)"
    )
...
selected_part = st.sidebar.selectbox(
    ...
    help=f"Analyze only this part number. Different parts have different tolerances, so analyzing separately prevents false alarms. ({len(unique_parts)} total part types)"
)

# Filter data to selected part
...
# ----------------------------
# Drill-down: Select a group and view its details + control charts
# ----------------------------
st.divider()
st.subheader("Drill-down: Select a group to view details and control charts")

chosen = st.selectbox(
    f"Select {group_col}",
    out["Group"].astype(str).unique(),
    help="Pick a group to see raw rows/values and control charts for that specific group."
)
...
# ----------------------------
# Control Charts for Selected Group Only
# ----------------------------
st.divider()
st.subheader(f"Control Charts for {group_col}: {chosen}")

# Get data for selected group only
selected_group_data = out[out["Group"].astype(str) == str(chosen)].iloc[0] if len(out[out["Group"].astype(str) == str(chosen)]) > 0 else None
...
if selected_group_data is not None:
    ...
    with col1:
        ...
        fig_x.add_trace(go.Scatter(
            x=[chosen],
            y=[selected_group_data["Xbar"]],
            mode="markers+text",
            name="Selected Group",
            marker=dict(size=12, color="blue"),
            text=[f"{selected_group_data['Xbar']:.4f}"],
        ...
        fig_x.add_trace(go.Scatter(
            x=[chosen, chosen],
            y=[selected_group_data["LCLx"], selected_group_data["UCLx"]],
            mode="lines",
            name="Control Limits",
        ...
        # Add limit values as text annotations
        fig_x.add_annotation(
            x=chosen, y=selected_group_data["UCLx"],
            text=f"UCL: {selected_group_data['UCLx']:.4f}",
            showarrow=False,
            yshift=10
        )
        fig_x.add_annotation(
            x=chosen, y=selected_group_data["LCLx"],
            text=f"LCL: {selected_group_data['LCLx']:.4f}",
            showarrow=False,
            yshift=-15
        )
        fig_x.add_annotation(
            x=chosen, y=Xbarbar,
            text=f"Center: {Xbarbar:.4f}",
            showarrow=False,
            xshift=50
        )
        ...
        # Statistics
        st.metric("Subgroup Size (n)", int(selected_group_data["n"]))
        st.metric("Xbar Value", f"{selected_group_data['Xbar']:.6f}")
        st.metric("Range (R)", f"{selected_group_data['R']:.6f}")
    
    with col2:
        st.write("**S Chart** (meaningful only if n ≥ 2)")
        
        if selected_group_data["n"] >= 2:
            fig_s = go.Figure()
            
            # Add the selected point
            fig_s.add_trace(go.Scatter(
                x=[chosen],
                y=[selected_group_data["S"]],
                mode="markers+text",
                name="Selected Group",
                marker=dict(size=12, color="blue"),
                text=[f"{selected_group_data['S']:.4f}"],
                textposition="top center"
            ))
            
            # Add control lines (if not NaN)
            if np.isfinite(selected_group_data["UCLs"]):
                fig_s.add_trace(go.Scatter(
                    x=[chosen, chosen],
                    y=[selected_group_data["LCLs"], selected_group_data["UCLs"]],
                    mode="lines",
                    name="Control Limits",
                    line=dict(color="red", dash="dash"),
                    showlegend=True
            ...
            if np.isfinite(selected_group_data["UCLs"]):
                fig_s.add_annotation(
                    x=chosen, y=selected_group_data["UCLs"],
                    text=f"UCL: {selected_group_data['UCLs']:.4f}",
                    showarrow=False,
                    yshift=10
                )
                fig_s.add_annotation(
                    x=chosen, y=selected_group_data["LCLs"],
                    text=f"LCL: {selected_group_data['LCLs']:.4f}",
                    showarrow=False,
                    yshift=-15
                )
            if np.isfinite(Sbar):
            ...
            # Statistics
            st.metric("S Value", f"{selected_group_data['S']:.6f}")
            if np.isfinite(Sbar):
                st.metric("S Chart Center (S̄)", f"{Sbar:.6f}")
        else:
            st.info("⚠️ S chart is not meaningful for n=1 (need at least 2 samples to calculate standard deviation)")
